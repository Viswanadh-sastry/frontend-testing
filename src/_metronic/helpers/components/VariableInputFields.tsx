...
interface IVariableInputFields {
  variables: any;
  setVariables: (variables: any) => void;
}

const VariableInputFields = ({ variables, setVariables }: IVariableInputFields) => {
  const [inputs, setInputs] = useState<{ key: string; value: any }[]>(convertObjectToArray(variables));
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    setVariables(convertArrayToObject(updatedInputs));
  };
...
const convertObjectToArray = (variables: any) => {
  const variablesArray = [];
  for (const key in variables) {
    variablesArray.push({ key: key, value: variables[key] });
  }
  return variablesArray;
};

const convertArrayToObject = (variables: any) => {
  const variablesObject: any = {};
  variables.forEach((item: any) => {
    variablesObject[item.key] = item.value;
  });
  return variablesObject;
};

export { VariableInputFields };
