...
interface IGatewayListPaginationProps {
  filterGateway: any;
  setFilterGateway: Dispatch<
    SetStateAction<{
      limit: number;
      offset: number;
      tenantId: string;
...
const GatewayListPagination = ({ filterGateway, setFilterGateway }: IGatewayListPaginationProps) => {
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterGateway.limit,
    links: [],
  });
  const gatewayListQuery = useQuery({
    queryKey: [`gatewayList`, filterGateway],
    queryFn: async () => getGateway(filterGateway).catch((error) => toast.error(error.message)),
    enabled: false,
  });
  const isLoading = gatewayListQuery.isLoading;

  useEffect(() => {
    if (gatewayListQuery.data) {
      const noOfLinks = gatewayListQuery.data.totalCount;
      const noOfPages = Math.ceil(noOfLinks / filterGateway.limit);
      const links = [];
  ...
  }, [gatewayListQuery.data]);

  const onChangePageSize = (e: any) => {
    setFilterGateway((prevState: any) => ({
      ...prevState,
  ...
  const updatePage = (page: number | undefined | null) => {
    ...
    updateState({ page, items_per_page: filterGateway.limit });
  };

  const PAGINATION_PAGES_COUNT = filterGateway.limit;
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_gateways_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${gatewayListQuery.data?.totalCount || 0} gateways`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_gateways_paginate">
