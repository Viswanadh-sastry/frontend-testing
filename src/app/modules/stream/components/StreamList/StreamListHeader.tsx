...
interface IStreamListHeaderProps {
  ...
  setStreamList: Dispatch<SetStateAction<any[]>>;
  streamList: any[];
  streamListQuery: any;
  pagination: any;
}

const StreamListHeader = ({ onShowAddStream, setCurrentPage, setPagination, setStreamList, streamList, streamListQuery, pagination }: IStreamListHeaderProps) => {
  ...
  const downloadCSV = async () => {
    ...
    const csvString = convertToCSV(streamList, headerOrder);
    const blob = new Blob([csvString], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "Stream-List.csv";
  ...
  const downloadXlsx = async () => {
    ...
    const formattedData = streamList.map((item: any) => {
  ...
  const downloadPDF = async () => {
    ...
    const data = streamList.map((item: any) =>
      headerOrder.map((key) => {
          ...
          <div className="d-flex align-items-center position-relative my-1">
            <input
              ...
              onChange={(e) => {
                ...
                setStreamList(
                  streamListQuery.data?.streams.filter((stream: any) => {
                    return stream.sql.toLowerCase().includes(e.target.value.toLowerCase());
                  })
                );
              }}
            />
