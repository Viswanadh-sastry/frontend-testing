...
const DeviceListHeader = ({ filterDevice }: IDeviceListHeaderProps) => {
  ...
  //   const { result: deviceList } = await getDevice({
  //     status: filterDevice.status,
  //   });
  ...
  //   const csvString = convertToCSV(deviceList, headerOrder);
  ...
  //   const formattedData = deviceList.map((item: any) => {
  ...
  //   const blob = new Blob([workbookBinary], { type: "device/octet-device" });
  ...
  //       }[key])
  //   );
  //   const data = deviceList.map((item: any) =>
  //     headerOrder.map((key) => {
  //       return item[key] || "";
  //     })
  //   );
  ...
  const onClickAddDevice = () => {
    navigate(`/applications/${id}/devices/add`);
  };
      ...
      <div className="card-header border-0 pt-6">
        ...
        <div className="card-toolbar">
          <div className="d-flex justify-content-end" data-kt-device-table-toolbar="base">
            <button type="button" className="btn btn-primary me-2 mx-2" onClick={onClickAddDevice}>
