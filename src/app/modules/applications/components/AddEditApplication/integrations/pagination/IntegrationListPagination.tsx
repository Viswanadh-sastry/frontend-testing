...
interface IIntegrationListPaginationProps {
  filterIntegration: any;
  setFilterIntegration: Dispatch<
    SetStateAction<{
      limit: number;
      offset: number;
...
const IntegrationListPagination = ({ filterIntegration, setFilterIntegration }: IIntegrationListPaginationProps) => {
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterIntegration.limit,
    links: [],
  });
  const integrationListQuery = useQuery({
    queryKey: [`integrationList`, filterIntegration],
    queryFn: async () => getIntegration(filterIntegration).catch((error) => toast.error(error?.response?.data?.message || "Something went wrong")),
    enabled: false,
  });
  const isLoading = integrationListQuery.isLoading;

  useEffect(() => {
    if (integrationListQuery.data) {
      const noOfLinks = integrationListQuery.data.totalCount;
      const noOfPages = Math.ceil(noOfLinks / filterIntegration.limit);
      const links = [];
  ...
  }, [integrationListQuery.data]);

  const onChangePageSize = (e: any) => {
    setFilterIntegration((prevState: any) => ({
      ...prevState,
  ...
  const updatePage = (page: number | undefined | null) => {
    ...
    updateState({ page, items_per_page: filterIntegration.limit });
  };
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_integrations_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${integrationListQuery.data?.totalCount || 0} integrations`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_integrations_paginate">
...
export { IntegrationListPagination };
