...
interface IDeviceProfileListHeaderProps {
  onShowImportApplication: () => void;
}

const ApplicationListHeader = ({ onShowImportApplication }: IDeviceProfileListHeaderProps) => {
  ...
  const downloadCSV = async () => {
    const { result: applicationList } = await getApplication({
      limit: 1000000000,
      offset: 0,
    ...
    const headerOrder = ["id", "name", "description", "createdAt"];
    const csvString = convertToCSV(applicationList, headerOrder);
    const blob = new Blob([csvString], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "Application-List.csv";
  ...
  const downloadXlsx = async () => {
    ...
    const formattedData = applicationList.map((item: any) => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "Application-List.xlsx";
  ...
  const downloadPDF = async () => {
    ...
    const data = applicationList.map((item: any) =>
      headerOrder.map((key) => {
        return item[key] || "";
      })
    );
    ...
    <>
      <div className="card-header border-0 pt-6">
        ...
        <div className="card-toolbar">
          <div className="d-flex justify-content-end" data-kt-application-table-toolbar="base">
            <button type="button" className="btn btn-primary me-2 mx-2" onClick={onClickAddApplication}>
              <KTIcon iconName="plus" className="fs-2" />
              Add Application
            </button>
            <button type="button" className="btn btn-light-primary me-2 mx-2" onClick={onShowImportApplication}>
