...
interface IApplicationListPaginationProps {
  filterApplication: any;
  setFilterApplication: Dispatch<
    SetStateAction<{
      limit: number;
      offset: number;
      tenantId: string;
...
const ApplicationListPagination = ({ filterApplication, setFilterApplication }: IApplicationListPaginationProps) => {
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterApplication.limit,
    links: [],
  });
  const applicationListQuery = useQuery({
    queryKey: [`applicationList`, filterApplication],
    queryFn: async () => getApplication(filterApplication).catch((error) => toast.error(error.message)),
    enabled: false,
  });
  const isLoading = applicationListQuery.isLoading;

  useEffect(() => {
    if (applicationListQuery.data) {
      const noOfLinks = applicationListQuery.data.totalCount;
      const noOfPages = Math.ceil(noOfLinks / filterApplication.limit);
      const links = [];
  ...
  const onChangePageSize = (e: any) => {
    setFilterApplication((prevState: any) => ({
      ...prevState,
  ...
  const updatePage = (page: number | undefined | null) => {
    ...
    updateState({ page, items_per_page: filterApplication.limit });
  };

  const PAGINATION_PAGES_COUNT = filterApplication.limit;
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_applications_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${applicationListQuery.data?.totalCount || 0} applications`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_applications_paginate">
...
export { ApplicationListPagination };
