...
interface IRuleListHeaderProps {
  ...
  setRuleList: Dispatch<SetStateAction<any[]>>;
  ruleList: any[];
  ruleListQuery: any;
  pagination: any;
}

const RuleListHeader = ({ onShowAddRule, setCurrentPage, setPagination, setRuleList, ruleList, ruleListQuery, pagination }: IRuleListHeaderProps) => {
  ...
  const downloadCSV = async () => {
    ...
    const csvString = convertToCSV(ruleList, headerOrder);
    const blob = new Blob([csvString], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "Rule-List.csv";
  ...
  const downloadXlsx = async () => {
    ...
    const formattedData = ruleList.map((item: any) => {
    ...
    const blob = new Blob([workbookBinary], { type: "application/octet-rule" });

    // Create a download link for the blob
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "Rule-List.xlsx";
  ...
  const downloadPDF = async () => {
    ...
    const data = ruleList.map((item: any) =>
      headerOrder.map((key) => {
          ...
          <div className="d-flex align-items-center position-relative my-1">
            <input
              ...
              onChange={(e) => {
                ...
                setRuleList(
                  ruleListQuery.data?.rules.filter((rule: any) => {
                    return rule.sql.toLowerCase().includes(e.target.value.toLowerCase());
                  })
                );
              }}
            />
