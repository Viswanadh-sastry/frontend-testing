...
interface IDeviceProfileListPaginationProps {
  filterDeviceProfile: any;
  setFilterDeviceProfile: Dispatch<
    SetStateAction<{
      limit: number;
      offset: number;
      tenantId: string;
...
const DeviceProfileListPagination = ({ filterDeviceProfile, setFilterDeviceProfile }: IDeviceProfileListPaginationProps) => {
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterDeviceProfile.limit,
    links: [],
  });
  const deviceProfileListQuery = useQuery({
    queryKey: [`deviceProfileList`, filterDeviceProfile],
    queryFn: async () => getDeviceProfile(filterDeviceProfile).catch((error) => toast.error(error.message)),
    enabled: false,
  });
  const isLoading = deviceProfileListQuery.isLoading;

  useEffect(() => {
    if (deviceProfileListQuery.data) {
      const noOfLinks = deviceProfileListQuery.data.totalCount;
      const noOfPages = Math.ceil(noOfLinks / filterDeviceProfile.limit);
      const links = [];
  ...
  }, [deviceProfileListQuery.data]);

  const onChangePageSize = (e: any) => {
    setFilterDeviceProfile((prevState: any) => ({
      ...prevState,
  ...
  const updatePage = (page: number | undefined | null) => {
    ...
    updateState({ page, items_per_page: filterDeviceProfile.limit });
  };

  const PAGINATION_PAGES_COUNT = filterDeviceProfile.limit;
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_deviceProfiles_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${deviceProfileListQuery.data?.totalCount || 0} device profiles`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_deviceProfiles_paginate">
...
export { DeviceProfileListPagination };
