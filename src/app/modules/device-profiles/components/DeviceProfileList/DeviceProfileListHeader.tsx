...
interface IDeviceProfileListHeaderProps {
  onShowImportDeviceProfile: () => void;
}

const DeviceProfileListHeader = ({ onShowImportDeviceProfile }: IDeviceProfileListHeaderProps) => {
  ...
  const downloadCSV = async () => {
    ...
    const headerOrder = ["id", "macVersion", "name", "regParamsRevision", "region", "supportsClassB", "supportsClassC", "supportsOtaa", "createdAt"];
    const csvString = convertToCSV(deviceProfileList, headerOrder);
    const blob = new Blob([csvString], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "Device-Profile-List.csv";
  ...
  const downloadXlsx = async () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "Device-Profile-List.xlsx";
  ...
  const downloadPDF = async () => {
    ...
    const doc = new jsPDF();
    const headerOrder = ["id", "macVersion", "name", "regParamsRevision", "region", "supportsOtaa"];
    const displayHeader = headerOrder.map(
      (key) =>
        ({
          ...
          name: "Name",
          regParamsRevision: "Reg Params Revision",
          region: "Region",
          supportsOtaa: "Supports OTAA",
        }[key])
    );
    ...
    const title = "Device Profile List";
    ...
    doc.save("Device-Profile-List.pdf");
  };
      ...
      <div className="card-header border-0 pt-6">
        ...
        <div className="card-toolbar">
          <div className="d-flex justify-content-end" data-kt-device-profile-table-toolbar="base">
            ...
            <button type="button" className="btn btn-light-primary me-2 mx-2" onClick={onShowImportDeviceProfile}>
